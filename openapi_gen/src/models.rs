...
use crate::{models, types::*};
    ...
    #[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize, validator::Validate)]
    #[cfg_attr(feature = "conversion", derive(frunk::LabelledGeneric))] 
    pub struct GetScoresPredataPathParams {
            /// 対象のゲームのID
                pub game_id: i32,
    }
...
#[derive(Debug, Clone, PartialEq, serde::Serialize, serde::Deserialize, validator::Validate)]
#[cfg_attr(feature = "conversion", derive(frunk::LabelledGeneric))]
pub struct GetScoresPredata200ResponseInner {
/// プレイヤーのID
    ...
    #[serde(rename = "scores")]
    #[serde(skip_serializing_if="Option::is_none")]
    pub scores: Option<Vec<models::Score>>,

}


impl GetScoresPredata200ResponseInner {
    #[allow(clippy::new_without_default, clippy::too_many_arguments)]
    pub fn new() -> GetScoresPredata200ResponseInner {
...
/// Converts the GetScoresPredata200ResponseInner value to the Query Parameters representation (style=form, explode=false)
/// specified in https://swagger.io/docs/specification/serialization/
/// Should be implemented in a serde serializer
impl std::fmt::Display for GetScoresPredata200ResponseInner {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let params: Vec<Option<String>> = vec![
...
/// Converts Query Parameters representation (style=form, explode=false) to a GetScoresPredata200ResponseInner value
/// as specified in https://swagger.io/docs/specification/serialization/
/// Should be implemented in a serde deserializer
impl std::str::FromStr for GetScoresPredata200ResponseInner {
    type Err = String;
    ...
    fn from_str(s: &str) -> std::result::Result<Self, Self::Err> {
        ...
        struct IntermediateRep {
            ...
            pub scores: Vec<Vec<models::Score>>,
        }
        ...
        while key_result.is_some() {
            let val = match string_iter.next() {
                Some(x) => x,
                None => return std::result::Result::Err("Missing value while parsing GetScoresPredata200ResponseInner".to_string())
            };
            ...
            if let Some(key) = key_result {
                ...
                match key {
                    ...
                    "scores" => return std::result::Result::Err("Parsing a container in this style is not supported in GetScoresPredata200ResponseInner".to_string()),
                    _ => return std::result::Result::Err("Unexpected key while parsing GetScoresPredata200ResponseInner".to_string())
                }
            }
        ...
        // Use the intermediate representation to return the struct
        std::result::Result::Ok(GetScoresPredata200ResponseInner {
            player_id: intermediate_rep.player_id.into_iter().next(),
            scores: intermediate_rep.scores.into_iter().next(),
        })
    }
}

// Methods for converting between header::IntoHeaderValue<GetScoresPredata200ResponseInner> and HeaderValue

#[cfg(feature = "server")]
impl std::convert::TryFrom<header::IntoHeaderValue<GetScoresPredata200ResponseInner>> for HeaderValue {
    type Error = String;

    fn try_from(hdr_value: header::IntoHeaderValue<GetScoresPredata200ResponseInner>) -> std::result::Result<Self, Self::Error> {
        let hdr_value = hdr_value.to_string();
        match HeaderValue::from_str(&hdr_value) {
             std::result::Result::Ok(value) => std::result::Result::Ok(value),
             std::result::Result::Err(e) => std::result::Result::Err(
                 format!("Invalid header value for GetScoresPredata200ResponseInner - value: {} is invalid {}",
                     hdr_value, e))
        }
    }
}
...
#[cfg(feature = "server")]
impl std::convert::TryFrom<HeaderValue> for header::IntoHeaderValue<GetScoresPredata200ResponseInner> {
    type Error = String;

    fn try_from(hdr_value: HeaderValue) -> std::result::Result<Self, Self::Error> {
        match hdr_value.to_str() {
             std::result::Result::Ok(value) => {
                    match <GetScoresPredata200ResponseInner as std::str::FromStr>::from_str(value) {
                        std::result::Result::Ok(value) => std::result::Result::Ok(header::IntoHeaderValue(value)),
                        std::result::Result::Err(err) => std::result::Result::Err(
                            format!("Unable to convert header value '{}' into GetScoresPredata200ResponseInner - {}",
                                value, err))
                    }
             },
             std::result::Result::Err(e) => std::result::Result::Err(
