    ...
    A: apis::games::Games + apis::players::Players + apis::scores::Scores + apis::votes::Votes + 'static,
{
    ...
    Router::new()
        ...
        .route("/scores/:game_id/pre",
            get(get_scores_predata::<I, A>)
        )
...
#[tracing::instrument(skip_all)]
fn get_scores_predata_validation(
  path_params: models::GetScoresPredataPathParams,
) -> std::result::Result<(
  models::GetScoresPredataPathParams,
), ValidationErrors>
{
  path_params.validate()?;
...
#[tracing::instrument(skip_all)]
async fn get_scores_predata<I, A>(
  method: Method,
  host: Host,
  cookies: CookieJar,
  Path(path_params): Path<models::GetScoresPredataPathParams>,
 State(api_impl): State<I>,
) -> Result<Response, StatusCode>
where
    I: AsRef<A> + Send + Sync,
    A: apis::scores::Scores,
{
  ...
  let result = api_impl.as_ref().get_scores_predata(
      method,
      host,
      cookies,
        path_params,
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::scores::GetScoresPredataResponse::Status200
                                                    (body)
                                                => {
                                                  let mut response = response.status(200);
                                                  {
                                                ...
                                                apis::scores::GetScoresPredataResponse::Status400_BadRequest
                                                => {
                                                  let mut response = response.status(400);
                                                  response.body(Body::empty())
                                                },
                                                apis::scores::GetScoresPredataResponse::Status404_NotFound
                                                => {
                                                  let mut response = response.status(404);
                                                  response.body(Body::empty())
                                                },
